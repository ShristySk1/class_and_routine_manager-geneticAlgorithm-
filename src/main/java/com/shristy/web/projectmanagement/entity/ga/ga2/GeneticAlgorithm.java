...
@Autowired
    private TimetableServiceImpl1 ttr;
@Autowired
    private TimetableRepository1 tr;
    private static Timetablenext timetable = new Timetablenext();
...
//					}
         ...
         TimetableServiceImpl1.truncate();
        try{
            display();
        }catch(Exception e){
...
//        if(TimetableServiceImpl.checkExist()){
     ...
     private static void display(List<String> ll) throws ClassNotFoundException, SQLException {
        // TODO Auto-generated method stub
...
System.out.print(" \t (" + lecture.getLecture().getSubject() + "#" + lecture.getLecture().getProfessor().getProfessorName() + ")");
                        if (z == 0) {
                            timetable.setSeven15eight05(lecture.getLecture().getSubject() + "#" + lecture.getLecture().getProfessor().getProfessorName()+"#"+lecture.getLecture().getStudentGroup().getName().split("/")[0]);
//                                  for(String name:ll){
//                                    String l= lecture.getLecture().getSubject();
//                                    
//                                      
//                         if(lecture.getLecture().getSubject().equals(name)){
//                             z++;z++;
//                         }
//                      }
                            z++;
